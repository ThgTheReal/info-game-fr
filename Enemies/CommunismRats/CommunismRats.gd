...
func _physics_process(delta):
	$ProgressBar.max_value = GlobalVariables.comunismRatMaxHealth
	$ProgressBar.value = GlobalVariables.comunismRatHealth
...
func take_damage(amount: int):
	...
	GlobalVariables.comunismRatHealth = GlobalVariables.comunismRatHealth - amount
	

		
func healthCheck() -> void:
	if GlobalVariables.comunismRatHealth <= 0:
		atack = false
		death = true
		GlobalVariables.comunismRatMaxHealth = GlobalVariables.comunismRatMaxHealth - health
...
func noPlayerNear(delta):
	...
	if $RayRight.is_colliding() or not $RayRightBottom.is_colliding():
		patrol_direction = -1
	elif $RayLeft.is_colliding() or not $RayLeftBottom.is_colliding():
		patrol_direction = 1
